...
void destruir_array(char **array, size_t cantidad);
bool iniciar_juego ();
...
int procesar_comando_basico(sala_t *sala, char **paramentros, size_t cant_parametros);
...
int main(int argc, char *argv[])
{
	...
	printf("Ya se puede comenzar el juego, " AMARILLO"¡buena suerte!\n" RESET_COLOR);
	...
	while(!sala_escape_exitoso(sala)){
		...
		if (!objetos_conocidos || !inventario){
			free(objetos_conocidos);
			free(inventario);
			fprintf(stderr, "Error al mostrar los objetos\n");
			return -1;
		}
		...
		if (cantidad_parametros < 3){
			procesado = procesar_comando_basico (sala,parametros,cantidad_parametros);
			if (procesado == SALIR){
				destruir_juego(sala, comandos, cantidad_comandos);
			...
			if (procesado > 0){
				destruir_array(parametros, cantidad_parametros);
				continue;
			}
		}
		...
		if (procesado == -1 && ejecutadas == 0){
			fprintf(stderr,ROJO"Acción u objeto inválido\n"RESET_COLOR);
		}
...
int procesar_comando_basico(sala_t *sala, char **parametros, size_t cant_parametros)
{
	if (cant_parametros == 1){
		if (!strcmp(parametros[0], "ayuda")){
			...
			destruir_array(parametros, cant_parametros);
			return SALIR;
		}
	}
	if (cant_parametros == 2){
...
{
	destruir_array(comandos,cantidad_comados);
	sala_destruir(sala);
}
...
bool iniciar_juego (){
	...
	char buff[MAX_LINEA];
	while (printf(">") && fgets(buff, MAX_LINEA, stdin) != NULL){
		convertir_a_minuscula(buff);
...
char **obtener_comandos_interacciones(const char *string, size_t *cantidad){
	...
	size_t i = 0;
	char **comandos = calloc(CAPACIDAD,sizeof(char *));
	if (!comandos)
		return NULL;
